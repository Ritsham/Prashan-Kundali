...
const Layout: React.FC = () => {
  ...
  const legalPagePaths = ['/return-policy', '/refund-policy', '/privacy-policy', '/disclaimer', '/about-contact'];
  const isLegalPage = legalPagePaths.some((path) => location.pathname === path || location.pathname === `${path}/`);
  ...
  const landingNavItems: NavItem[] = [
    ...
    { label: 'Astro Community', to: '/astro-community', external: true, targetBlank: true },
  ];
  const navItems = isLegalPage ? landingNavItems : appNavItems;
  const policyItems = [
  ...
  return (
    <div className={`shell ${isCommunityWorkspace ? 'community-app-shell' : ''} ${isLegalPage ? 'legal-shell' : ''}`} id="main-app">
