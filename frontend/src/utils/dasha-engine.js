...
export class DashaWidget {
  ...
  render() {
    ...
    this._bindBack();
    if (this.variant !== 'workstation') this._renderSummary();
  ...
  _renderLevel(level, rows, parentPath, pushStack = true) {
    ...
    if (table) {
      if (this.variant === 'workstation') {
        if (heading) heading.textContent = `${DASHA_NAMES[level] || level}s`;
        table.innerHTML = `
          <thead>
            <tr>
              <th>Planet</th>
          ...
          <tbody>
            ${rows.map((period, index) => {
              const active = this._isCurrentPeriod(level, period);
              return `
                <tr data-index="${index}" data-next="${next}" class="${isClickable ? 'dasha-row-clickable' : ''} ${active ? 'is-active-dasha' : ''}">
                  <td>
                    <strong>${esc(period.lord)}</strong>
                    ${active ? '<span class="dasha-active-badge">Active</span>' : ''}
                    ${isClickable ? '<span class="dasha-row-drill">▶</span>' : ''}
                  </td>
  ...
  _isCurrentPeriod(level, period) {
    const keyByLevel = {
      maha: 'current_mahadasha',
      antara: 'current_antardasha',
      pratyantara: 'current_pratyantardasha',
      sookshma: 'current_sookshma',
      prana: 'current_prana',
    };
    const current = this.dasha?.[keyByLevel[level]];
    return Boolean(current && current.lord === period.lord);
  }
