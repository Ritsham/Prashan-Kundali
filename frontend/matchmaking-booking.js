...
function renderBooking(context, consultant, queue) {
  ...
  root.innerHTML = `
    ...
    <div class="booking-form-block">
      ...
      <label>Choose consultation slot
...
function bookingPayload(context) {
  return {
    ...
    preferred_slot: root.querySelector("#booking-slot")?.value || "Consult now",
...
function buildSlots() {
  ...
  const now = new Date();
  [2, 6, 24, 30, 48].forEach(hours => {
    slots.push(formatter.format(new Date(now.getTime() + hours * 60 * 60 * 1000)));
  });
  return slots;
}
